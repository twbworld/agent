...
系统的知识库（来源于Chatwoot的 `canned_responses`）同步和管理遵循一套多层、健壮的策略，以平衡实时性、性能和数据一致性。

#### 5.1.1. 定时审计同步
-   **机制**: 一个每日执行的定时任务 (`KeywordReloader`)，负责从 Chatwoot 拉取全量数据，并与本地缓存（Redis, ChromaDB, 内存Map）进行比对。
-   **作用**: 作为系统的“审计员”，它可以修复任何因意外情况（如手动修改Chatwoot后台）导致的数据不一致，是数据**最终一致性**的终极保障。

#### 5.1.2. API触发的实时缓存更新
当管理员通过系统提供的管理页面（`keyword.html`）对知识条目进行增、删、改操作时，会触发更精细的实时缓存策略，以提供即时反馈：

-   **删除 (Delete)**: 采用 **实时精准清理** 策略。
    -   系统会立即从内存、Redis 和 ChromaDB 中精确删除对应的条目，用户操作即刻生效。

-   **新增/修改 (Upsert)**: 采用“**实时精准更新 + 延迟重同步校准**”的混合模式。
    1.  **实时精准更新**: API 服务接收到请求后，会**立即**对新增或修改的条目执行完整的处理流程（包括调用LLM生成标准问题、向量化），并将其精准地更新到所有缓存中。这确保了用户的操作能够“即时生效”。
    2.  **延迟重同步校准**: 在执行实时更新后，系统会启动一个带**防抖机制**的延迟任务（10分钟）。如果在延迟期内有连续的新操作，计时器会不断重置。当操作停止10分钟后，系统会自动触发一次完整的“定时审计同步”任务，作为最终的校准和检查。

这种混合模式兼顾了用户体验（即时反馈）和系统鲁棒性（最终一致性），同时通过防抖机制避免了因频繁操作导致的服务过载。

### 5.2. 对话处理流程 (Webhook 触发)
...
3.  **智能分诊 (Triage & Routing)**:
    ...
    -   **意图规划 (Qwen3:Small)**: 将用户原始问题、以及上一步获取的上下文信息，发送给小参数的Qwen模型（分诊台）。该模型被要求对用户输入的意图、情绪、相关性、紧急度进行综合判断，并返回一个结构化的JSON。
        -   **高风险识别**: 分诊台会特别识别涉及**高额资损**（如金额>1000元争议）或**重大投诉**的内容，并标记为高紧急度（`critical/high`）。
    -   **智能路由**: AI编排服务根据分诊台返回的JSON结果执行路由：
        -   如果判断为**情绪激动**、**明确要求转人工**或**高紧急度（高风险）**，则立即转接人工客服，且不设置AI接管宽限期。
        -   如果判断为**无关问题**或**闲聊**，则返回礼貌拒绝的固定回复，并终止流程。
        -   如果分诊通过，则进入下一步深度处理。
4.  **复杂路径 (RAG + LLM & Tool Use)**:
    -   **获取历史记录**: 系统将获取完整的对话历史（并按轮数修剪）。
    -   **结果生成与工具调用 (Qwen3:Large)**: 将用户原始问题、完整的向量搜索结果（作为RAG参考资料）、对话历史发送给大参数Qwen模型。
    -   **Agentic Loop (工具执行循环)**:
        -   如果LLM返回 `<tool_code>` 标签，系统将解析并调用 MCP 服务执行对应工具（如查询订单）。
        -   工具执行结果会被追加到对话历史中，系统**再次**调用 LLM 进行结果综合。
        -   LLM 根据工具结果生成最终的人性化回复。
5.  **转人工逻辑**: 在以下情况，系统将自动调用 Chatwoot API 转接人工客服：
    -   用户明确提出“转人工”（由分诊台或关键词匹配捕获）。
    -   分诊台识别到用户有强烈负面情绪或问题紧急度高（含高风险业务）。
    -   大型LLM在生成答案时，判断无法回答（返回 `I_AM_UNSURE...` 信号）。
    -   用户第三次询问同一个AI无法解决的问题。
    -   消息包含不支持的格式（如图片、音频）。
