...
// GetDashboardDetails 从MCP获取仪表板详情 (商品或订单)，用于在Chatwoot仪表板应用中展示
func (p *DashboardApi) GetDashboardDetails(ctx *gin.Context) {
	...
	details, err := service.Service.UserServiceGroup.DashboardService.GetDetails(ctx, req.GoodsID, req.OrderID)
	if err != nil {
